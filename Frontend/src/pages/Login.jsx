...
const LoginPage = () => {
  ...
  let redirectTo = new URLSearchParams(location.search).get('redirect') || '/dashboard';
  let jobId = new URLSearchParams(location.search).get('jobId');

  // If jobId wasn't passed as a separate query param, it may be embedded
  // inside the `redirect` value (e.g. "/candidate-drop-cv?jobId=123").
  // Extract it and normalize `redirectTo` to be the path only.
  if (!jobId && redirectTo && redirectTo.includes('?')) {
    try {
      const parts = redirectTo.split('?');
      const qp = new URLSearchParams(parts[1]);
      const parsedJobId = qp.get('jobId');
      if (parsedJobId) {
        jobId = parsedJobId;
        redirectTo = parts[0];
      }
    } catch (e) {
      // ignore parse errors and fall back to original values
    }
  }

  const redirectToWithJobId = jobId ? `${redirectTo}?jobId=${jobId}` : redirectTo;
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (res && res.needsPassword) {
        // Redirect user to signup page so they can set a password
        const params = new URLSearchParams();
        if (res.email) params.set('email', res.email);
        if (res.fullName) params.set('fullName', res.fullName);
            ...
            <div className="relative bg-black/40 backdrop-blur-xl rounded-2xl p-8 border border-white/[0.05] shadow-2xl overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
                ...
                >
                  Not Registered Yet?{' '}
                  <button
                    onClick={() => navigate(`/signup?redirect=${encodeURIComponent(redirectToWithJobId)}`)}
