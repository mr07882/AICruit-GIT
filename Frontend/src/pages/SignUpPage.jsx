...
const SignUpPage = () => {
  ...
  React.useEffect(() => {
    try {
      ...
      const fn = qp.get('fullName');
      if (e) {
        setFormData(prev => ({ ...prev, email: e }));
        setInfoMsg('Please set a password to complete signup for this account.');
      }
      if (fn) setFormData(prev => ({ ...prev, fullName: fn }));
    } catch (e) {}
  }, [location.search]);
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (res && res.needsPassword) {
        setFormData(prev => ({ ...prev, email: res.email || prev.email, fullName: res.fullName || prev.fullName }));
        setInfoMsg('Please set a password to complete signup for this account.');
        setIsLoading(false);
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await authService.requestOtp({ email, fullName, password });
      setInfoMsg('OTP sent to your email. Please enter it below.');
      if (res && res.preview) {
        setInfoMsg(prev => prev + ` (preview: ${res.preview})`);
      }
      setStep('otp');
    } catch (err) {
  ...
  const handleResendOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await authService.resendOtp({ email: formData.email, fullName: formData.fullName, password: formData.password });
      setInfoMsg('OTP resent to your email.');
      if (res && res.preview) setInfoMsg(prev => prev + ` (preview: ${res.preview})`);
    } catch (err) {
              ...
              {step === 'form' && (
                <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
                  ...
                  >
                    <div className="relative flex items-center overflow-hidden rounded-lg">
                      ...
                      <input
                        ...
                        required
                        className="w-full bg-white/5 border border-white/10 focus:border-white/20 text-white placeholder:text-white/30 h-10 transition-all duration-300 pl-10 pr-3 focus:bg-white/10 rounded-lg outline-none"
                      />
                  ...
                  >
                    <div className="relative flex items-center overflow-hidden rounded-lg">
                      ...
                      <input
                        ...
                        onBlur={() => setFocusedInput(null)}
                        required
                        className="w-full bg-white/5 border border-white/10 focus:border-white/20 text-white placeholder:text-white/30 h-10 transition-all duration-300 pl-10 pr-10 focus:bg-white/10 rounded-lg outline-none"
                      />
                  ...
                  >
                    ...
                    <div className="relative overflow-hidden bg-white/5 text-white font-medium h-10 rounded-lg border border-white/10 hover:border-white/20 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              {step === 'otp' && (
                <form onSubmit={handleVerifyOtp} className="space-y-4 relative z-10">
                  ...
                  <div className="text-sm text-white/80">
                    Enter the 6-digit OTP sent to <strong className="text-white">{formData.email}</strong>
                  </div>
                  ...
                  >
                    <div className="relative flex items-center overflow-hidden rounded-lg">
                      <Lock className={`absolute left-3 w-4 h-4 transition-all duration-300 ${
                        focusedInput === "otp" ? 'text-white' : 'text-white/40'
                      }`} />
                  ...
                  <div className="flex gap-3 mt-6">
                    ...
                    >
                      ...
                      <div className="relative overflow-hidden bg-white text-black font-medium h-10 rounded-lg transition-all duration-300 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed">
                        <AnimatePresence mode="wait">
                          {isLoading ? (
                            <motion.div
                              key="loading"
                    ...
                    >
                      ...
                      <div className="relative overflow-hidden bg-white/5 text-white font-medium h-10 rounded-lg border border-white/10 hover:border-white/20 transition-all duration-300 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  <motion.button
                    ...
                    className="w-full text-xs text-white/60 hover:text-white/80 transition-colors duration-200"
                  >
                    Back to sign up
                  </motion.button>
                </form>
